...
module.exports.run = async function ({ api, event, args, Users, permssion, getText ,Threads}) {
  ...
  const listNDH = NDH || config.NDH ||  [];
  {
    ...
    const data = [
      ...
      "Mtx dropped.",
      "MintBot là MtxProject nhưng module là idea của SpermBot.",
...
◈─────────────────💚✨\n♪♪♪♪♪♪♪『${namebot}』.♪♪♪♪♪♪♪\n◈─────────────────💚✨\n» Prefix system: ${PREFIX}\n» Prefix box: ${prefix}\n» Modules: ${commands.size}\n» Ping: ${Date.now() - dateNow}ms\n» Total users: ${global.data.allUserID.length} \n» Total threads: ${global.data.allThreadID.length} ◈─────────────────💚✨\n╭───────────╮\n🌻 𝙤𝙬𝙣𝙚𝙧   𝑼𝒛𝒂𝒊𝒓  🌻\n╰───────────╯ ╭────────────╮\n🥀 (𝑴𝑻𝑿 💚✨)      💞 \n╰────────────╯\n🍇 Kìrâñ RajPööt ☠️ 🏴‍☠️ 🍇\n◈─────────────────💚✨ https://www.facebook.com/Mtxuzair\n◈─────────────────💚✨`, attachment: fs.createReadStream(__dirname + "/uzair/kensu.jpg"), }, event.threadID, () => fs.unlinkSync(__dirname + "/uzair/kensu.jpg"));
      return request(encodeURI(link[Math.floor(Math.random() * link.length)])).pipe(fs.createWriteStream(__dirname + "/uzair/kensu.jpg")).on("close", () => callback()); 
  }
};