...
if (!fs.existsSync("./Koja-Jutt.json")) {
  log.warn("warn", "koja-uptime-fca File Not Found Create New")
    global.fca = new Object ({
    data: new Object ({
    languages: "en",
    mainName: "[ MTX-PROJECT ]",
    mainColor: "#00FFFF",
...
fs.writeFileSync("./Koja-Jutt.json", JSON.stringify(global.fca.data, null, "\t"))
  return process.exit(1)
}
try {
      var langfile = JSON.parse(fs.readFileSync(__dirname + "/languages/languages.json", 'utf-8'));
var lang
    switch (require("../../Koja-Jutt.json").languages) {
      ...
      case "jp": lang = langfile.jp.index;
        break;
      default: {
      log.warn("warn", "Only 4 Kinds of Languages ​​VI, EN, TH & JP Currently Supported, Self Fix By Finding And Deleting Koja-Jutt.json File")
      process.exit(0)
      }
    }
  } catch(e) {
...
global.fca = new Object({
     ObjFcaConfig: require("../../Koja-Jutt.json"),
     languages: lang
})
...
axios.get(`https://Uptimerobot.edu-choru.repl.co/uptime?link=${UrlRepl}`)
		logger(global.fca.languages.uptime + UrlRepl, '[ MTX-PROJECT ]');
	}
	catch {
		logger(global.fca.languages.erroUptime, '[ MTX-PROJECT ]');
  }
}
...
function buildAPI(globalOptions, html, jar) {
    ...
    logger(`${global.fca.languages.loginSu}${userID}`, "[ MTX-PROJECT ]");
    ...
    if (oldFBMQTTMatch) {
        ...
        logger(`${global.fca.languages.region}${region}`, "[ MTX-PROJECT ]");
    } else {
...
logger(global.fca.languages.login, "[ MTX-PROJECT ]");
        return utils
            ...
            .then(function(res) {
                ...
                if (headers.location.indexOf('https://www.facebook.com/checkpoint/') > -1) {
                    logger(global.fca.languages.errorFa, "[ MTX-PROJECT ]");
                                            ...
                                            utils
                                                ...
                                                .then(res => {
                                                    try {
                                                        ...
                                                        clearInterval(checkVerified);
                                                        logger(global.fca.languages.okweb, "[ MTX-PROJECT ]");
                                                        if (callback === prCallback) {
                                                            callback = function(err, api) {
                                                                if (err) return prReject(err);
                                                                return prResolve(api);
...
require('dotenv').config({ path: './../.env' });
                ...
                var ans = makeid(49)
                    process.env["FBKEY"] = ans;
                        fs.writeFile('./../.env', `FBKEY=${ans}`, function (err) {
                            if (err) {
                            logger(global.fca.languages.errorEnv, "[ MTX-PROJECT ]");
                    }
                else logger(global.fca.languages.env,"[ MTX-PROJECT ]")
        }); 
    }
    catch (e) {
        console.log(e);
        logger(global.fca.languages.errorPassRandom, "[ MTX-PROJECT ]");
    }
}
    ...
    if (process.env['FBKEY']) {
        try {
            ...
            if (appState.includes('[')) {
                logger(global.fca.languages.appstateDec, '[ MTX-PROJECT ]');
            } else {
                try {
                    appState = JSON.parse(appState);
                    var StateCrypt = require('./StateCrypt');
                    appState = StateCrypt.decryptState(appState, process.env['FBKEY']);
                    logger(global.fca.languages.appstate, '[ MTX-PROJECT ]');
                }
                catch (e) {
                    logger(global.fca.languages.appstateError, '[ MTX-PROJECT ]');
                }
            }
        }
        catch (e) {
    ...
    try {
        ...
        try {
            ...
            return logger(global.fca.languages.appstateError, '[ MTX-PROJECT ]')
        }
    }
      }
    try { 
    ...
    mainPromise
        .then(function() {
            logger(global.fca.languages.oklogin, "[ MTX-PROJECT ]");
                    //!---------- Auto Check, Update START -----------------!//
                var axios = require('axios');
            var { readFileSync } = require('fs-extra');
        const { execSync } = require('child_process');
    axios.get('https://raw.githubusercontent.com/ChoruTiktokers182/fb-anya/main/package.json').then(async (res) => {
            ...
            if (localbrand != res.data.version) {
                        ...
                        log.warn("UPDATE > ",`${global.fca.languages.autoUpdate}`);
                            try {
                                ...
                                logger(global.fca.languages.restart, '[ MTX-PROJECT ]');
                    ...
                    logger(`${global.fca.languages.checkVersion}` + localbrand + ' !', "[ MTX-PROJECT ]");      
                         logger(global.fca.languages.chucAdmin, "[ MTX-PROJECT ]");
