...
module.exports.run = async function ({ event, api, args, permssion, Users }) {
    ...
    switch (args[0]) {
        ...
        case "reset": {
            ...
            return api.sendMessage("Reset the entire warning list!", threadID, messageID);
        }
        default: {
            if (permssion != 2) {
                ...
                try {
                    ...
                    if (args.length == 0) return api.sendMessage("You have not entered a reason for the warning!", threadID, messageID);
                    ...
                    return api.sendMessage(`Warned ${name} with reason: ${args.join(" ")}, ${(data.banned) ? `Because I have been warned 3 times, the above account has been banned` : `The above account is still available ${data.warningLeft} warning turn!`}`, threadID, messageID);
