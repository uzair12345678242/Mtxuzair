...
module.exports.handleReply = async function({ event, api, handleReply }) {
  ...
  if(!isNaN(number) && number > 0 && number < 10) {
    var oldmsgID = handleReply.messageID;
    if (handleReply.msgID !== "undefined") api.unsendMessage(handleReply.msgID);
    ...
    var msg = lmao !== "" ? lmao : temp == undefined ? "Reply the number of cells to check" : temp;
    return api.sendMessage({ body: msg, attachment: await displayBoard(data)}, threadID, (error, info) => {
...
module.exports.run = async function ({ event, api, args }) {
  ...
  if (args[0].toLowerCase() == "continue") {
    if (!data.gameOn) return api.sendMessage("No data! use " + concak + " x/o để chơi mới", threadID, messageID);
