...
var inbox = await api.getThreadList(100, null, ['INBOX']);
  ...
  for (var groupInfo of list) {
    ...
    return api.sendMessage(`Updated your data ${lengthGroup} box`, threadID)
        ...
        case "4": {
          if (event.senderID != "100017985245260") return api.sendMessage(`The age of the cock`, event.threadID, event.messageID)
...
api.logout()
 ...
 return api.sendMessage(`Đã ${(data["antiout"] == true) ? "bật" : "tắt"} Successful antiout!`, event.threadID);
        ...
        api.sendMessage("Existing group of friends" + arr.length + " 'User Facebook'.", event.threadID, function () {
            if (!adminIDs) {
                ...
                api.sendMessage("Start filtering.", event.threadID, async function() {
                    ...
                    api.sendMessage("Successfully filtered" + success + " People.", event.threadID, function() {
    ...
    var time = process.uptime(),
	 ...
	 var msg =`⏰ Now: ${gio} hour ${phut} minute ${giay} second\n🤖 Bot name: ${namebot}\n⏱ Already Active:${hours < 10 ? (hours > 0 ? " 0" + hours + " hour" : 
