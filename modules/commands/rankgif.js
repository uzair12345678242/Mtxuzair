...
module.exports.run = async({ event, api, args, Currencies, Users }) => {
    ...
    var id = listUserID
      ...
      const rank = exp.findIndex(info => parseInt(info.uid) == parseInt(event.senderID)) + 1;   const infoUser = exp[rank - 1];
        ...
        if (rank == 0) return api.sendMessage("You are not currently in the database so cannot see your rankings, please try again in 5 seconds.", event.threadID, event.messageID);
