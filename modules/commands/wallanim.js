...
module.exports.run = async function({ api, args, event, permssion }) {
  ...
  if(args[0] == "find" || args[0] == "tìm"){
    ...
    var msg = ({
      body: `CHARACTER ID ${args[1]}, DEFAULT COLOR ${lengthchar.colorBg}`,
      attachment: t
    })
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  if (event.senderID != handleReply.author) return api.sendMessage('“C', event.threaID);
  ...
  const Canvas = require('canvas');
    ...
    if(handleReply.step == 1){
      ...
      return api.sendMessage(`You have selected the char with the serial number ${event.body}, Reply to this message to enter your name`, event.threadID, (err, info) => {
    return global.client.handleReply.push({
      step: 2,
      name: this.config.name,
      chartid: event.body,
      ...
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(`You have chosen your name as ${event.body}, Reply to this message to enter the color you want`, event.threadID, (err, info) => {
    return global.client.handleReply.push({
      step: 3,
      name: this.config.name,
