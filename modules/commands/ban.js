...
module.exports.run = async function({ api, args, Users, event, Threads, utils, client }) {
	...
	if (!fs.existsSync(__dirname + `/uzairmtx/bans.json`)) {
			const dataaa = {warns: {}, banned: {}};
			fs.writeFileSync(__dirname + `/uzairmtx/bans.json`, JSON.stringify(dataaa));
					}
  var bans = JSON.parse(fs.readFileSync(__dirname + `/uzairmtx/bans.json`)); //read file contents
  /*
  {warns: {}, banned: {tid: []}};
  */
  if(!bans.warns.hasOwnProperty(threadID)) {
			bans.warns[threadID] = {}; 
			fs.writeFileSync(__dirname + `/uzairmtx/bans.json`, JSON.stringify(bans, null, 2));
  ...
  var reason = "";
		  ...
		  var arraytag = [];
		  var arrayname = [];
		  //Check if you have been warned yet
